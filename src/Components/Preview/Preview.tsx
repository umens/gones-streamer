...
class Preview extends React.Component<PreviewProps, PreviewState> {
  ...
  resizeListener = async () => {
    ...
    let timeoutwidth = setTimeout(async () => 
      await this.setState({ width: this.getWidth() }),
     150);
    await this.setState({ timeoutId: timeoutwidth });
  };
  ...
  componentWillUnmount = async () => {
    try {
      ...
      if (this.state.timeoutId) {
        clearTimeout(this.state.timeoutId);
        await this.setState({ timeoutId: undefined });
      }      
  ...
  getScreenshot = async () => {
    try {
      if (this.props.display) {
        let height: number = ( this.state.width / 16 ) * 9; // 16:9
        let data = await this.props.ObsRemote.getScreenshot({ imageWidth: this.state.width, imageHeight: Math.round(height) });
        if (data && data.img) {
